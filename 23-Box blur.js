...
// integers in the blurred output. To get the first 
...
function solution(image) {
    ...
    for (let i = 1; i < image.length - 1; i++) {
        for (let j = 1; j < image[i].length - 1; j++) {
            aTmp.push(Math.floor((image[i - 1][j - 1] + image[i - 1][j] +
                image[i - 1][j + 1] + image[i][j - 1] + image[i][j] +
